...
import {
  ...
  AlignRight, AlignJustify, EyeOff, FolderPlus, Trash2, Calendar, Image as ImageIcon,
  Paperclip, FileText, Code, ArrowUp, Loader2, Copy, Clipboard, Sliders,
  Bold, Italic, Underline, Printer, Share2, Save
} from 'lucide-react';
...
export const TableEditor = ({
  ...
  }, [selectedId, setSpreadsheetDialog]);

  const toggleCellFormat = useCallback((field: 'bold' | 'italic' | 'underline') => {
    ...
    setCellMetadata(prev => {
      ...
      const isPartofSelection = selection &&
        activeCell.row >= Math.min(selection.startRow, selection.endRow) &&
        activeCell.row <= Math.max(selection.startRow, selection.endRow) &&
        (() => {
          ...
          const currentColIdx = localVisibleHeaders.indexOf(activeCell.col);
      ...
      if (isPartofSelection && selection) {
        ...
        for (let r = minRow; r <= maxRow; r++) {
          for (let c = minColIdx; c <= maxColIdx; c++) {
            ...
            next[key] = { ...next[key], [field]: !next[key]?.[field] };
          }
        }
      } else {
        const mIdx = masterColumnOrder.indexOf(activeCell.col);
        const key = activeCell.row === -1 ? `header:${activeCell.col}` : toA1Key(activeCell.row, mIdx);
        next[key] = { ...next[key], [field]: !next[key]?.[field] };
      }
      return next;
    });
  }, [activeCell, selection, allHeaders, hiddenColumns, masterColumnOrder, saveStateToHistory, setCellMetadata]);

  const isActiveCellFormat = useCallback((field: 'bold' | 'italic' | 'underline') => {
    if (!activeCell) return false;
    const mIdx = masterColumnOrder.indexOf(activeCell.col);
    const key = activeCell.row === -1 ? `header:${activeCell.col}` : toA1Key(activeCell.row, mIdx);
    return !!cellMetadata[key]?.[field];
  }, [activeCell, masterColumnOrder, cellMetadata]);
                ...
                return (
                  <>
                    ...
                    <div className="px-2 py-1 flex items-center justify-between mx-1 gap-2" onMouseEnter={() => setContextMenu((prev: any) => prev ? { ...prev, showFonts: false, showFormats: false, showNumberFormats: false, showFormulaFormats: false } : null)}>
                      <span className="text-[10px] font-bold text-muted-foreground tracking-wider uppercase">Align</span>
                      <div className="flex bg-muted/20 p-0.5 rounded-md border border-border/30 w-36">
                        {(['left', 'center', 'right', 'justify'] as const).map((a) => {
                          ...
                          return (
                            <button
                              ...
                              title={`${a.charAt(0).toUpperCase() + a.slice(1)} Alignment`}
                              className={`flex-1 py-1 flex justify-center items-center rounded transition-colors ${active
                                ? 'bg-card text-accent shadow-sm border border-border/20 font-medium'
                                : 'text-muted-foreground hover:text-foreground'
                                }`}
                            >
